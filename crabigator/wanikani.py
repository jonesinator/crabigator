...
class WaniKani():
    ...
    def get_level_progression(self):
        return self._get('level-progression', 'requested_information', LEVEL_PROG)
    level_progression = property(get_level_progression)

    def get_srs_distribution(self):
        return self._get('srs-distribution', 'requested_information', SRS_DISTS)
    ...
    def _get_items(self, levels, resource, fields):
        response = self._raw_request(resource,
            argument=','.join([str(level) for level in levels]))
    ...
    def _get_meta_items(self, resource, argument):
        ...
        for item in response['requested_information']:
            tmap = {'radical':RADICAL, 'kanji':KANJI, 'vocabulary':VOCABULARY}
